...
class GameDetailScreen:
    ...
    def __init__(self, screen: pygame.Surface, config_manager,
        ...
        self.scraper_svc = ScraperService(config_manager)
        ...
        # Welke knop is geselecteerd: BTN_PLAY, BTN_COVER, of BTN_SCRAPER
        self._btn_focus   = BTN_PLAY

        # Cover picker overlay
        self._picker: CoverPicker | None = None
        # Scraper overlay
        self._scraper: "ScraperScreen | None" = None
    ...
    def update(self, dt: float):
        ...
        # Scraper actief? Geef acties door.
        if self._scraper is not None:
            result = self._scraper.update(dt, actions)
        ...
        for action in actions:
            if action == ACTION_BACK:
                ...
                # Schakel tussen Spelen, Cover wijzigen, en Scraper
                if self._btn_focus == BTN_PLAY:
                    self._btn_focus = BTN_COVER
                elif self._btn_focus == BTN_COVER:
                    self._btn_focus = BTN_SCRAPER
                else:
                    self._btn_focus = BTN_PLAY
                ...
                elif self._btn_focus == BTN_SCRAPER:
                    from scraper.screen_scraper import ScraperScreen
                    self._scraper = ScraperScreen(
                        self.screen, self.cfg, self.input,
                        self.scraper_svc, self.console, self.game,
                        parent_screen=self
                    )
    ...
    def draw(self):
        ...
        # Scraper tekent zichzelf bovenop alles
        if self._scraper is not None:
            # Teken eerst de achtergrond
            self._draw_base()
            self._scraper.draw()
    ...
    def _draw_base(self):
        ...
        # ── Knop: Cover scrapen ─────────────────────────────────────────────────
        scraper_sel  = (self._btn_focus == BTN_SCRAPER)
        scraper_rect = pygame.Rect(info_x, info_y, 240, 44)
        _draw_rr(self.screen,
                 COLOR_BTN2_SEL if scraper_sel else COLOR_BTN2,
                 scraper_rect, 10)
        pygame.draw.rect(self.screen, COLOR_BTN_BRD,
                         scraper_rect, width=2 if scraper_sel else 1,
                         border_radius=10)
        scraper_lbl = self._font_sub.render("\U0001f50d  Cover scrapen", True,
                                              COLOR_TEXT if scraper_sel else COLOR_TEXT_DIM)
        self.screen.blit(scraper_lbl, scraper_lbl.get_rect(center=scraper_rect.center))
        info_y += 58
